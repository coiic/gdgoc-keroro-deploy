...
(() => {
  ...
  async function loadMyApps() {
    if (!me) { myApps = []; return false; }
    try {
      const { data, error } = await sb.rpc('my_applications');
      if (error || !Array.isArray(data)) {
        console.error(error || 'Invalid application response');
        return false;
      }
      myApps = data;
      return true;
  ...
  function renderMine() {
    ...
    list.innerHTML = myApps.length ? myApps.map((a) => {
      ...
      const acts = (closed
        ...
        : `<button type="button" data-swap="${a.id}">${3 - a.rank}지망으로 바꾸기</button>`
          + `<a href="${esc(t.url ?? '#')}">신청서 보기·수정 →</a>`)
        + `<button type="button" data-drop="${a.id}">취소</button>`;
      return `<div class="mine-item"><div><span class="rank">${a.rank}지망</span> <strong>${esc(t.title ?? a.team_slug)}</strong> <span class="muted">· ${roles}</span></div><div class="acts">${acts}</div></div>`;
  ...
  async function openMine() {
    ...
    if (!(await loadMyApps())) {
      alert('신청 내역을 불러오지 못했습니다. 잠시 뒤 다시 열어 주세요.');
      return;
    }
    ...
    switch (btn.dataset.act) {
      ...
      case 'delete':
        ...
        try {
          ...
          if (error) { alert(friendly(error.message)); return; }
        } catch (error) {
          alert('삭제 결과를 확인하지 못했습니다. 잠시 뒤 다시 확인해 주세요.');
          console.error(error);
          return;
        }
  ...
  function syncRank(form) {
    ...
    if (block) block.hidden = $('[name=rank]:checked', form)?.value !== '1';
  }
  ...
  function openApplyForm(app) {
    ...
    if (app) {
      ...
      form.role_second.value = app.role_second ?? '';
  ...
  async function renderTeam() {
    ...
    if (form) {
      ...
      form.onsubmit = async (e) => {
        ...
        if (!motivation) return fail('지원하는 이유를 적어 주세요.');
        btn.disabled = true;
        try {
          const editing = editId;
          if (!editing && !(await ensureCard())) return;
          if (!(await loadMyApps())) return fail('신청 내역을 확인하지 못해 저장을 중단했습니다. 잠시 뒤 다시 시도해 주세요.');
          if (!editing && myApps.length >= 2) return fail('이미 두 팀에 신청했습니다. 내 신청 현황에서 정리한 뒤 다시 신청해 주세요.');

          const rank = Number($('[name=rank]:checked', form).value);
          const clash = myApps.find((a) => a.rank === rank && a.id !== editing);
          if (clash) {
            // 순위 변경과 신청 저장을 별도 요청으로 처리하면 일부만 저장될 수 있다.
            // 서버 트랜잭션이 제공되기 전까지 순위 변경은 전용 set_rank 흐름에서만 한다.
            return fail('선택한 순위에 다른 신청이 있습니다. 내 신청 현황에서 순위를 먼저 변경한 뒤 다시 저장해 주세요.');
          }
          ...
          if (rank === 1) {
            try { fallbackError = await saveFallback($('#fallback-block', form)); }
            catch (error) { fallbackError = error; }
          }
          form.reset();
          await refresh();
          if (fallbackError) {
            alert('신청서는 저장됐지만 미배정 시 선택은 저장하지 못했습니다. 내 신청 현황에서 해당 선택을 다시 저장해 주세요.');
          }
        } catch (error) {
          console.error(error);
          fail('저장 결과를 확인하지 못했습니다. 내 신청 현황을 확인한 뒤 다시 시도해 주세요.');
        } finally {
          btn.disabled = false;
        }
      };
    ...
    if (done) {
      $('#edit-apply').onclick = async () => {
        if (!(await loadMyApps())) {
          alert('신청서를 불러오지 못했습니다. 기존 내용 보호를 위해 수정을 열지 않았습니다. 다시 시도해 주세요.');
          return;
        }
        const application = myApps.find((a) => a.id === mine.id);
        if (!application) { await refresh(); return; }
        openApplyForm(application);
      };
  ...
  async function refresh() {
    ...
    await Promise.all([renderTeam(), renderIndex()]);
  }
